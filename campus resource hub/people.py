...
from managing import Resource, AuditMixin
...
class Mentor(Person, AuditMixin):    
    def approve_request(self, resource: Resource, student: 'Student') -> bool:
        if resource.status == 'available':
            self.log_action(f"APPROVAL: Approved {resource.type} for {student.name}")
            return True
        self.log_action(f"DENIAL: Denied {resource.type} for {student.name} (Unavailable)")
        return False
...
class Student(Person, AuditMixin):
    def __init__(self, name: str):
        super().__init__(name)
        self._progress = 0
    ...
    def enroll(self, course: Course) -> None:
        ...
        self.log_action(f"ENROLLED: {self.name} -> {course.title} (Mentor: {course.mentor.name})")
    ...
    def borrow_resource(self, resource: Resource, mentor: Mentor) -> None:
        if mentor.approve_request(resource, self):
            ...
            self.log_action(f"BORROW: Resource {resource.id}")

    def needs_resource(self) -> bool:
        return len(self.borrowed_resources) < 2


class PremiumStudent(Student, AuditMixin):

    def enroll(self, course: Course) -> None:
        super().enroll(course) 
        self.log_action(f"PREMIUM ENROLL: {self.name} -> {course.title} (Mentor: {course.mentor.name})")
